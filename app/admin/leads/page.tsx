      ...
      <div className="max-w-7xl mx-auto px-4 py-8">
        ...
        {loading ? (
          <div className="text-center py-12">
            <div className="text-lg" style={{ color: '#ff9500' }}>טוען...</div>
          </div>
        ) : error ? (
          <div className="text-center py-12">
            ...
            <div className="p-6 border-b" style={{ borderColor: 'rgba(138, 138, 138, 0.15)' }}>
              <div className="grid grid-cols-4 gap-4">
                <div className="text-center">
                  <div className="text-3xl font-bold" style={{ color: '#ff9500' }}>{leads.length}</div>
            ...
            <div className="overflow-x-auto">
              <table className="w-full">
                <thead className="border-b" style={{ borderColor: 'rgba(138, 138, 138, 0.15)' }}>
                  <tr>
                    <th className="px-4 py-3 text-right text-xs font-semibold" style={{ color: '#ff9500' }}>שם</th>
                    <th className="px-4 py-3 text-right text-xs font-semibold" style={{ color: '#ff9500' }}>מייל</th>
                    <th className="px-4 py-3 text-right text-xs font-semibold" style={{ color: '#ff9500' }}>השקעה</th>
                    <th className="px-4 py-3 text-right text-xs font-semibold" style={{ color: '#ff9500' }}>סכום</th>
                    <th className="px-4 py-3 text-right text-xs font-semibold" style={{ color: '#ff9500' }}>טווח</th>
                    <th className="px-4 py-3 text-right text-xs font-semibold" style={{ color: '#ff9500' }}>סיכון</th>
                    <th className="px-4 py-3 text-right text-xs font-semibold" style={{ color: '#ff9500' }}>תאריך</th>
                  </tr>
                </thead>
                <tbody>
                  {leads.map((lead, idx) => (
                    <tr key={lead.id} className="border-b" style={{ 
                      ...
                      backgroundColor: idx % 2 === 0 ? 'rgba(255, 149, 0, 0.02)' : 'transparent'
                    }}>
                      ...
                      <td className="px-4 py-3 text-sm" style={{ color: '#ff9500' }}>{lead.investmentName}</td>
